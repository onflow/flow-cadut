...
/**
* Method to generate cadence code for getNFTs script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNFTsTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNFTs =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNFTs = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNFTsTemplate(addressMap);

  reportMissing("arguments", args.length, 2, `getNFTs =>`);
