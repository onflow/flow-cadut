...
/**
* Method to generate cadence code for getNFTIDs script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNFTIDsTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNFTIDs =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNFTIDs = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNFTIDsTemplate(addressMap);

  reportMissing("arguments", args.length, 1, `getNFTIDs =>`);
